...
class CollectionOfPhotos:  # contains dataframe with all photographs
    ...
    def plot_values(self, key, mode=None, bar_nr = None):
        '''
        ...
        if mode == 'line':
            ...
            self.plot_pie(key, mode)
        elif mode == 'bar':
            self.plot_bar(key, mode, bar_nr)
        elif mode == 'donut':
            self.plot_donut(key, mode)
        else:
    ...
    def plot_bar(self, key, mode, bar_nr):
        ...
        sns.barplot(x=top_value_counts.index, y=top_value_counts.values, palette=("Blues_r"))
