...
class CollectionOfPhotos:  # contains dataframe with all photographs
    ...
    def plot_values(self, key, mode=None, bar_nr = None, legend=None):
        '''
        ...
        if mode is None:
            mode = 'line'
        if mode == 'line':
            self.plot_line(key, mode)
        elif mode == 'pie':
            self.plot_pie(key, mode, legend)
        elif mode == 'bar':
            self.plot_bar(key, mode, bar_nr)
        elif mode == 'donut':
            self.plot_donut(key, mode, legend)
        else:
            raise ValueError('Please input correct mode')

    def plot_line(self, key, mode):
        value_counts = self.get_value_counts(key, mode)
    ...
    def plot_pie(self, key, mode, legend=False):
        value_counts = self.get_value_counts(key, mode)
        value_labels = value_counts.index
        plt.figure(figsize=(8, 8))
        if legend is False or legend is None:
            plt.pie(value_counts, labels=value_labels, autopct='%1.1f%%', startangle=140, colors = sns.color_palette('muted'))
        else:
    ...
    def plot_donut(self, key, mode, legend=False):
        value_counts = self.get_value_counts(key, mode)
        value_labels = value_counts.index
        plt.figure(figsize=(8, 8))
        plt.title(f'Fraction of Photos by {key}')
    ...
    def plot_bar(self, key, mode, bar_nr):
        if bar_nr == '':
            bar_nr = 25
